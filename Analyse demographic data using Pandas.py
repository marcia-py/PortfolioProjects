...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv('adult.data.csv', na_values =['','?','-'], sep = ',', decimal = ',')
    df = df.round(1) #To round all numeric columns to the nearest tenth
    ...
    highest_earning_country_percentage = (highest_earning_country / df['native-country'].value_counts()) * 100

    # Identify the most popular occupation for those who earn >50K in India.
    df_high_earning = df[df['salary'] == '>50K']
    IN_high_earning = df_high_earning[df_high_earning['native-country'] == 'India']
    top_IN_occupation = IN_high_earning['occupation'].value_counts().idxmax()
